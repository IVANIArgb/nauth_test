...
function renderLesson(lesson) {
    ...
    if (isEditMode && (currentUserRole === 'admin' || currentUserRole === 'super_admin')) {
        // Загружаем редактор урока
        if (typeof initLessonEditor === 'function') {
            initLessonEditor(lesson.id);
        }
    }
}
...
function renderContentBlock(block, index) {
    ...
    switch (block.block_type) {
        ...
        case 'video':
            ...
            // Если URL уже полный путь, используем его
            if (videoUrl && videoUrl.startsWith('/categories-data/')) {
                // URL уже правильный, ничего не делаем
            } else if (videoUrl && !videoUrl.startsWith('http') && !videoUrl.startsWith('/')) {
        ...
        case 'file':
            ...
            if (!fileUrl || fileUrl.startsWith('http') || fileUrl.startsWith('/categories-data/')) {
                // URL уже правильный, ничего не делаем
            } else if (fileUrl && currentLesson && currentLesson.path_identifier) {
                // Относительный путь или только имя файла - формируем полный путь
        ...
        case 'image':
            ...
            if (!imageUrl || imageUrl.startsWith('http') || imageUrl.startsWith('/categories-data/')) {
                // URL уже правильный, ничего не делаем
            } else if (imageUrl && currentLesson && currentLesson.path_identifier) {
                // Относительный путь или только имя файла - формируем полный путь
            ...
            } else if (!imageUrl && imageFilename && currentLesson && currentLesson.path_identifier) {
                // Только filename, без url
...
async function loadBreadcrumbs(lessonId) {
    try {
        ...
        if (breadcrumbsContainer && data?.category && data?.course && data?.lesson) {
...
function addActionButtons(lesson) {
    ...
    if (!actionsContainer) {
        ...
        if (pageTitle && pageTitle.parentNode) {
            pageTitle.parentNode.insertBefore(actionsContainer, pageTitle.nextSibling);
        } else {
    ...
    if (currentUserRole === 'admin' || currentUserRole === 'super_admin') {
        ...
        editBtn.innerHTML = '✏️ Редактировать урок';
        editBtn.onclick = () => {
            window.location.href = `/lessons-content-pg?lesson_id=${lesson.id}&edit=true`;
        };
    ...
    askQuestionBtn.innerHTML = '❓ Задать вопрос по уроку';
...
async function submitTest(blockId) {
    if (!currentLesson || !currentLesson.id) {
        ...
        if (blockEl) {
            ...
            customAlert('Урок не загружен', 'Ошибка');
        }
        return;
    }
    const blockEl = document.querySelector(`.content-block[data-block-id="${blockId}"]`);
    if (!blockEl) {
        customAlert('Блок теста не найден', 'Ошибка');
        return;
    }
...
function showError(message) {
    ...
    if (lessonContentContainer) {
        ...
        // Если контейнер не найден, используем alert как fallback
