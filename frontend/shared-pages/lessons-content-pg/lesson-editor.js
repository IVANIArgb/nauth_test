 ...
 * Оборачиваем в IIFE, чтобы не конфликтовать с другими скриптами.
 */
...
async function handleDroppedFiles(files) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            showError(`Не удалось загрузить файл «${file.name}»: ${error?.message || 'запрещённый тип/расширение'}`);
        }
    }
        ...
        switch (blockType) {
            ...
            case 'text': {
                ...
                if (document.getElementById('text-as-link').checked) {
                    ...
                    if (raw) {
                        // Нормализация ссылок: поддерживаем /путь, полный URL, а также относительные пути типа "questions-pg"
                        if (/^https?:\/\//i.test(raw) || raw.startsWith('/')) {
                            linkUrl = raw;
                        } else {
                            try {
                                const loc = window.location;
                                // Если начинается с текущего хоста без схемы
                                if (raw.toLowerCase().startsWith(loc.host.toLowerCase())) {
                                    linkUrl = `${loc.protocol}//${raw}`;
                                } else {
                                    // Считаем относительным путём внутри сайта
                                    linkUrl = '/' + raw.replace(/^\/+/, '');
                                }
                            } catch (e) {
                                linkUrl = raw;
